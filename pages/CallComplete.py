...
st.title("🛠️ WJS Report Generator (Advanced)")
st.caption(
    "Auto-extract (Place/RDL + WJS Label) • WJS BF presets • SST BF (email+report) • SN auto-fill via photos"
)
st.divider()
...
"""
    ...
    try:
        ...
        return {"place_name": place, "rdl": f"RDL: {rdl_digits}" if rdl_digits else "RDL: "}
...
"""
    ...
    try:
        ...
        return {"item": _strip(data.get("item")), "pn": _strip(data.get("pn")), "sn": _strip(data.get("sn"))}
...
# =====================
# AI VISION: EXTRACT SERIAL NUMBER FROM COMPONENT PHOTO (SST BF)
# =====================
def extract_serial_from_component_photo(img_bytes: bytes) -> str:
    """
    Extract ONLY a serial number/ID from a component label photo.
    ...
    system_prompt = """
You are extracting a serial number from a photo of a hardware label.
...
Rules:
- sn must be exactly what is visible (alphanumeric allowed).
- Do not invent.
- If not visible, return empty string.
...
"""
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": system_prompt},
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "Extract ONLY the serial number (SN) from this photo."},
                        {"type": "image_url", "image_url": {"url": data_url}},
                    ],
                },
            ],
        ...
        sn = sn.replace("SN:", "").replace("S/N:", "").strip()
        return sn
    except Exception as e:
        st.error(f"Error extracting component SN: {e}")
        return ""
...
def format_wjs_info(item: str, pn: str, sn: str) -> str:
    ...
    if pn:
        lines.append(f"- P/N: {pn}")
    if sn:
        lines.append(f"- S/N: {sn}")
    return "\n".join(lines)
...
# hashes to prevent re-extracting same images
if "sst_photo_hashes" not in st.session_state:
    st.session_state.sst_photo_hashes = {}
...
with st.form("report_form"):
    report_type = st.selectbox("📄 Report Type", ["PM", "INSTALLATION", "DEINSTALLATION", "WJS BF", "SST BF"])
    ...
    # =====================
    # SST BF UI (inside the form) — TEXT INPUTS
    # Photos are handled OUTSIDE the form for instant extraction.
    # =====================
    sst_items: List[Dict[str, str]] = []
    if report_type == "SST BF":
        st.markdown("### 🔁 SST BF - Components to Replace (Old SN / New SN)")
        st.caption("Choose component(s), then upload OLD/NEW photos below to auto-fill SNs instantly.")
    ...
    descricao = st.text_area("📝 Details (facts only — what happened / issues / time / actions taken)", placeholder=details_placeholder)
    ...
    extras_custom = st.text_area("➕ Other materials (one per line)", placeholder="Cable ties\nVelcro straps\nEthernet coupler")

    submitted = st.form_submit_button("✅ Generate")

# =====================
# SST BF: Add/Remove + INSTANT photo extraction (OUTSIDE form)
# =====================
    ...
    with cB:
        if st.button("➖ Remove last", key="btn_remove_comp"):
            if len(st.session_state.sst_replacements) > 1:
                ...
                st.rerun()
    with cC:
        st.caption("Add/remove here, then fill inside the form above. Upload photos below to auto-fill SNs.")
    st.divider()

    st.subheader("📷 SST BF - Upload Photos to Auto-fill Serial Numbers")
    if not client:
        st.warning("OPENAI_API_KEY not set — SN auto-extract disabled.")
    else:
        for idx, row in enumerate(st.session_state.sst_replacements):
            st.markdown(f"**Component #{idx+1}**")
            ...
            with colp1:
                old_photo = st.file_uploader(
                    f"Upload OLD component photo #{idx+1}",
                    type=["png", "jpg", "jpeg"],
                    key=f"sst_old_photo_{idx}",
                )
            with colp2:
                new_photo = st.file_uploader(
                    f"Upload NEW component photo #{idx+1}",
                    type=["png", "jpg", "jpeg"],
                    key=f"sst_new_photo_{idx}",
                )

            if old_photo is not None:
                b = old_photo.getvalue()
                h = _hash_bytes(b)
                hk = f"old_{idx}"
                if h and st.session_state.sst_photo_hashes.get(hk) != h:
                    with st.spinner(f"Extracting OLD SN for Component #{idx+1}..."):
                        sn = extract_serial_from_component_photo(b)
                        if sn:
                            row["old_sn"] = sn
                            st.session_state[f"sst_old_{idx}"] = sn
                    st.session_state.sst_photo_hashes[hk] = h

            if new_photo is not None:
                b = new_photo.getvalue()
                h = _hash_bytes(b)
                hk = f"new_{idx}"
                if h and st.session_state.sst_photo_hashes.get(hk) != h:
                    with st.spinner(f"Extracting NEW SN for Component #{idx+1}..."):
                        sn = extract_serial_from_component_photo(b)
                        if sn:
                            row["new_sn"] = sn
                            st.session_state[f"sst_new_{idx}"] = sn
                    st.session_state.sst_photo_hashes[hk] = h
...
if submitted:
    ...
    if cat5:
        itens_extras.append("CAT5 Network Cable: L63")
    if extension:
        itens_extras.append("Power Extension: L38A")
    if power_supply:
        itens_extras.append("Power Supply: L64")
    if power_cord:
        itens_extras.append("Power Cord: L1")
    if transceiver:
        itens_extras.append("Transceiver: L61")
    if transpower:
        itens_extras.append("Transceiver Power: L62")
    if powerbar:
        itens_extras.append("Power Bar: L38")
    if steel:
        itens_extras.append("Steel Cable: L48")
    if chain:
        itens_extras.append("Hanging Chain: L46")
    ...
    wjs_info_block = ""
    if report_type in ["INSTALLATION", "DEINSTALLATION"]:
        wjs_info_block = format_wjs_info(st.session_state.wjs_item, st.session_state.wjs_pn, st.session_state.wjs_sn)
    ...
    if report_type == "PM":
        ...
        # Use the latest values (session_state is source of truth because photos can update it)
        normalized_items: List[Dict[str, str]] = []
        for idx, row in enumerate(st.session_state.sst_replacements):
            comp_choice = st.session_state.get(f"sst_choice_{idx}", row.get("choice", "Slip Reader"))
            row["choice"] = comp_choice

            # component name
            if comp_choice == "Other (type manually)":
                comp_name = _strip(st.session_state.get(f"sst_comp_{idx}", row.get("component", "")))
            else:
                comp_name = comp_choice

            old_sn = _strip(st.session_state.get(f"sst_old_{idx}", row.get("old_sn", "")))
            new_sn = _strip(st.session_state.get(f"sst_new_{idx}", row.get("new_sn", "")))
            ...
            if comp_name or old_sn or new_sn:
                normalized_items.append({"component": comp_name, "old_sn": old_sn, "new_sn": new_sn})
        ...
        with c2:
            st.download_button(
                "⬇️ Download Email (.txt)", data=email_text, file_name="sst_bf_email_request.txt", mime="text/plain"
            )
        ...
        with c4:
            st.download_button(
                "⬇️ Download Report (.txt)", data=report_text, file_name="sst_bf_replacement_report.txt", mime="text/plain"
            )
        ...
        st.download_button(
            "⬇️ Download Both (.txt)", data=combined, file_name="sst_bf_email_and_report.txt", mime="text/plain"
        )
